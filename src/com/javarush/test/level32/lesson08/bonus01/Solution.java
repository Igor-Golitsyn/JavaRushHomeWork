...
public class Solution {
    ...
    public <T extends Item> T getProxy(Class<T>cclass, Class<?>... interfases) {
        ArrayList<Class<?>> interUP=new ArrayList<>(Arrays.asList(interfases));
        if (!interUP.contains(cclass))interUP.add(cclass);
        Class<?>[] cl=new Class[interUP.size()];
        interUP.toArray(cl);
        Object item=Proxy.newProxyInstance(cclass.getClassLoader(),cl,new ItemInvocationHandler());
        return (T) item;
    }
