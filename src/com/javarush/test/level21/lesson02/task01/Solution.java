...
public class Solution {
    ...
    public static void print(byte[] bytes) {
        String binLine=new String();
        for (int i = 0; i < bytes.length; i++) {
            binLine=String.valueOf(Integer.toBinaryString(bytes[i]));
            while (binLine.length()<32){
                binLine="0".concat(binLine);
            }
            System.out.print(binLine.substring(24, 32) + " ");
        }
        System.out.println();
    }
}
