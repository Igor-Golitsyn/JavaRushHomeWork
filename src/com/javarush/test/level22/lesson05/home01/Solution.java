...
public class Solution {
    ...
    public String getPartOfString(String string, String threadName) {
        String substr;
        try {
            substr = string.substring(string.indexOf(9)+1,string.lastIndexOf(9));
        }
        catch (Exception e) {
            if (Thread.currentThread().getName().equals(FIRST_THREAD_NAME))throw new TooShortStringFirstThreadException(e);
            if (Thread.currentThread().getName().equals(SECOND_THREAD_NAME))throw new TooShortStringSecondThreadException(e);
